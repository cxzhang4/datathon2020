...
# Sum "DispoIncome" = (taxable income)-(income tax) by zip code
summed_zip_income <- irs_zip_data %>%
  mutate(DispoIncome = (A04800 - A06500)) %>%
  select(c(zipcode, DispoIncome)) %>%
  mutate(zipcode = as.factor(zipcode)) %>%
  group_by(zipcode) %>%
  summarise(DispoIncome = sum(DispoIncome))

# Join summed (taxable income)-(income tax) by zip code
